...
impType <- impute(train, classes = list(factor = imputeMode(), integer = imputeMean()),
...
cmpImputations <- data.frame(select(impRPart$data, Self_Employed,LoanAmount,LoanAmount.dummy), 
...
imputeMLR <- function(){
  ...
  impute= function(data, target, col, values, var) {
    ...
    nv <- sapply(ed, function(y) {
      values %>% 
        filter(Education == y ) %>% 
        select(mean_LoanAmount) %>% 
        unlist %>% 
        unname } ) 
  ...
  args  = list(var=c("Education"))
  )
}

impCustom <- impute(train, cols=list(LoanAmount=imputeMLR()))

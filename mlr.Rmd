...
impSimple <- impute(train, 
              cols = list(Self_Employed = imputeMode(), Amount = imputeMean()), 
              dummy.cols = c("Self_Employed","Amount"), 
              dummy.type = "numeric")
...
unique(impType$data$Amount[as.logical(impType$data$Amount.dummy)])

testType<-reimpute(test,impType$desc)

unique(testType$LoanAmount[is.na(test$Amount)])
...
impRPart <- impute(train[,-1], 
                    target = "Status",
                    cols = list(Amount = imputeLearner(makeLearner("regr.rpart"))
...
cmpImputations <- data.frame(select(impRPart$data, Amount, Amount.dummy), select(impSimple$data, Amount))

ggplot(filter(cmpImputations,Amount.dummy==T)) + geom_point(aes(x=Amount, y=Amount.1))
...
impCustom <- impute(train, cols=list(Amount=imputeByCategory("Education")), 
                    dummy.cols = c("Amount"), dummy.type = "numeric")

```

```{r}
unique(impCustom$data$Amount[as.logical(impCustom$data$Amount.dummy)])

testCustom <- reimpute(test, impCustom$desc)

unique(testCustom$Amount[is.na(test$Amount)])
```
...
trainTask <- makeClassifTask(data = train, target = "Status", positive = "Y")

test$Status <- as.factor(0)
testTask <- makeClassifTask(data = test, target = "Status")
...
trainTask <- dropFeatures(task = trainTask, features = c("ID","Married.dummy"))

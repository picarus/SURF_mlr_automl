...
The object return by impute has two parts, data that contains the new data:
...
And desc that contains the params of the imputation:
...
mboControl <- setMBOControlInfill(mboControl, crit = makeMBOInfillCritAEI()) # expected improvement
...
If I am not mislead not all combinations are possible... like Tune Params and Feature Selection... if all the parameters are to be recovered, because only benchmark and resample allow the detailed results collection.  TBC
...
resFull <- benchmark(task = trainTask, 
               learners = list(lrn, rf), 
               resampling = outer,
               measures = auc,
               show.info = F)
